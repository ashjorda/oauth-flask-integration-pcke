...
def generate_code_verifier(length=50):
    """
    Generate a 'code_verifier' (random string) of 'length' bytes, which is URL-safe.
    The length must be between 43 and 128 characters. We'll use 128 for maximum entropy.
    """
...
def generate_code_challenge(code_verifier):
    """
    Generate the 'code_challenge' from the 'code_verifier' using SHA256 hashing
    and then base64 URL-safe encoding.
    """
...
def get_tokens_refresh():
    ...
    payload = ("grant_type=refresh_token&client_id={0}&client_secret={1}&"
               "refresh_token={2}&code_verifier={3}").format(clientID, secretID, session['refresh_token'], code_verifier)
...
def main_page():
    ...
    """Main Grant page"""
    return render_template("index.html", state=state_secret, code_verifier=code_verifier, code_challenge=code_challenge)
...
def get_tokens(code):
    ...
    payload = ("grant_type=authorization_code&client_id={0}&client_secret={1}&"
               "code={2}&redirect_uri={3}&code_verifier={4}").format(clientID, secretID, code, redirectURI, code_verifier)
    req = requests.post(url=url, data=payload, headers=headers)
    results = json.loads(req.text)
    print(results)
...
@app.route("/studeninfo", methods=['GET'])
def studentinfo():
    print("function : studentinfo()")
    ...
    student_information = response.json()
    ...
    return render_template("student_info.html", studentinfo=student_information)
...
@app.route("/recordinginfo", methods=['GET'])
def recordinginfo():
    print("function : recording_info()")
    print("accessing token ...")
    recordings = recording_info()

    print("status code : ", recordings.status_code)
    # Do a check on the response. If the access_token is invalid then use refresh
    # token to obtain a new set of access token and refresh token.
    if recordings.status_code == 401:
        get_tokens_refresh()
        recordings = recording_info()

    r = recordings.json()
    print(r)
    # print("response status code : ", recordings.status_code)
    # recording_list = []
    # for i in range(len(r)):
    #     recording_list.append(r[i]['topic'])

    return render_template("recordings.html", recordings=r)
